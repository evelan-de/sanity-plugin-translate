...
const SyncDocumentMediaAction = ({
  ...
  const translateAndSyncOtherDocuments = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}${API.SYNC_DOCUMENT_MEDIA}`, {
        method: 'POST',
        headers: {
      ...
      if (isTranslated) {
        ...
        toast.push({
          status: 'success',
          title: t('syncDocumentsMediaAction.toast.success.title'),
          description: t('syncDocumentsMediaAction.toast.success.description'),
          duration: 4000,
        });
      }
    } catch (error) {
      toast.push({
        status: 'error',
        title: t('syncDocumentsMediaAction.toast.error.title'),
        description: t('syncDocumentsMediaAction.toast.error.description'),
        duration: 4000,
      });
  ...
  return {
    label: loading
      ? t('syncDocumentsMediaAction.label.loading')
      : t('syncDocumentsMediaAction.label'),
...
export default SyncDocumentMediaAction;
